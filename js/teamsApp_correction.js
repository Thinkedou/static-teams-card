...
function setCardInfo(who,card){
    ...
    const jobTitre = card.querySelector('.card-text .job')
    const fullName = card.querySelector('.card-text h2')
    const laius = card.querySelector('.card-text p')
    const profile  = card.querySelector('.card-image')




    jobTitre.textContent = jobTitle
    fullName.textContent = firstName+' '+lastName
    laius.textContent = verbatim
    profile.style.background=`url(img/${profilePic})`
